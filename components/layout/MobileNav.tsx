...
const navItems = [
  ...
  },
  {
    ...
    icon: (_active: boolean) => (
      <div className="w-12 h-12 rounded-2xl bg-linear-to-br from-violet-600 to-indigo-600 flex items-center justify-center shadow-lg -mt-5">
...
export default function MobileNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 bg-white/90 backdrop-blur-md border-t border-slate-100 lg:hidden">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                item.center ? 'pb-0' : '',
                !item.center && (active ? 'text-violet-600' : 'text-slate-400')
              )}
            >
              {item.icon(active)}
              {!item.center && (
                <span className={cn('text-[10px] font-medium', active ? 'text-violet-600' : 'text-slate-400')}>
                  {item.label}
                </span>
              )}
            </Link>
